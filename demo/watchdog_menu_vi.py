...
def print_status(status: Status) -> None:
    print()
    print("========== TRẠNG THÁI WATCHDOG ==========")
    print(f"STATUS raw      : 0x{status.raw:08X}")
    print(f"EN_EFFECTIVE    : {status.en_effective}  ({'đang bật' if status.en_effective else 'đang tắt'})")
    print(f"FAULT_ACTIVE    : {status.fault_active}  ({'ĐANG FAULT' if status.fault_active else 'không fault'})")
    print(f"ENOUT           : {status.enout}")
    print(f"WDO             : {status.wdo}  ({'bình thường' if status.wdo else 'assert fault/reset'})")
    print(f"LAST_KICK_SRC   : {status.last_kick_src}  ({status.last_kick_src_name})")
    print("=========================================")


def print_board_hint() -> None:
    print()
    print("Gợi ý LED/nút trên board:")
    print("  S1: nút kick watchdog ở chế độ button")
    print("  S2: enable phần cứng")
    print("  D3: fault/WDO")
    print("  D4: ENOUT/watchdog đang monitor")
    print()
...
def show_checklist(state: AppState) -> None:
    print()
    print("================ CHECKLIST TEST ================")
    for item in state.results.values():
        print(f"{item.name:<32} : {item.result}")
        if item.detail:
            print(f"  - {item.detail}")
    print("================================================")
    print()
    print("Cấu hình hiện tại trong menu:")
    print(f"  PORT   : {state.port}")
    print(f"  BAUD   : {state.baud}")
    print(f"  tWD    : {state.twd_ms} ms")
    print(f"  tRST   : {state.trst_ms} ms")
    print(f"  ARM    : {state.arm_us} us")
...
def print_menu(state: AppState) -> None:
    print()
    print("===================================================")
    print("        MENU DEMO WATCHDOG KIWI 1P5 - TIẾNG VIỆT   ")
    print("===================================================")
    print(f"Port: {state.port} | Baud: {state.baud} | tWD={state.twd_ms} ms | tRST={state.trst_ms} ms")
    print("---------------------------------------------------")
    print(" 1. Xem trạng thái hiện tại")
    print(" 2. Cấu hình thời gian demo")
    print(" 3. Demo nút S1/S2")
    print(" 4. Test UART kick tự động")
    print(" 5. Xóa fault")
    print(" 6. Disable watchdog")
    print(" 7. Theo dõi trạng thái liên tục")
    print(" 8. Raw read/write register")
    print(" 9. Chạy flow demo khuyến nghị")
    print("10. Xem checklist PASS/FAIL")
    print("11. Lưu báo cáo test ra file")
    print(" 0. Thoát")
    print("===================================================")
