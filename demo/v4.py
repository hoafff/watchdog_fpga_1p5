...
Đã cập nhật các tính năng nâng cao: Auto-Reconnect, Data Logging CSV, Save Config.
"""
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r") as f:
                data = json.load(f)
                state.twd_ms = data.get("twd_ms", state.twd_ms)
                state.trst_ms = data.get("trst_ms", state.trst_ms)
                state.arm_us = data.get("arm_us", state.arm_us)
        except Exception:
            pass
...
def action_setcfg(state: AppState) -> None:
    print("\nNhập thông số demo. Bấm Enter để dùng giá trị cũ/mặc định.")
    state.twd_ms = ask_int("tWD_ms (Mô phỏng mode CWD=NC)", state.twd_ms)
    state.trst_ms = ask_int("tRST_ms (Thời gian giữ WDO khi fault)", state.trst_ms)
    state.arm_us = ask_int("arm_delay_us (Cửa sổ ignore WDI khi vừa enable)", state.arm_us)
...
def action_live_monitor(state: AppState) -> None:
    print(color("\n[LIVE MONITOR] Theo dõi trạng thái liên tục.", "cyan"))
    print("Tính năng nâng cấp: Tự động kết nối lại (Auto-Reconnect) và Ghi log ra CSV.")
    print("Nhấn Ctrl+C để dừng.\n")

    os.makedirs("logs", exist_ok=True)
    csv_path = os.path.join("logs", f"monitor_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")

    dev = None
    try:
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["Time", "RAW_HEX", "EN", "FAULT", "ENOUT", "WDO", "SRC"])
            
            while True:
                try:
                    dev = connect_once(state)
                    sys.stdout.write(f"\r{color('[KẾT NỐI]', 'green')} Đã kết nối thành công tới {state.port}         \n")
                    sys.stdout.flush()
                    ...
                    sys.stdout.write(f"\r{color('[CẢNH BÁO]', 'yellow')} Mất kết nối tới {state.port}. Đang tự động kết nối lại...     ")
                    sys.stdout.flush()
                    time.sleep(2)
                except Exception as e:
                    sys.stdout.write(f"\r{color('[LỖI TRUYỀN THÔNG]', 'red')} {e} - Thử lại sau 1s...                  ")
                    sys.stdout.flush()
                    time.sleep(1)
                finally:
                    if dev:
        ...
        print(color(f"\n\nĐã dừng theo dõi. Dữ liệu CSV được lưu tại: {csv_path}", "green"))
...
def action_register_dump(state: AppState) -> None:
    ...
    try:
        dev = connect_once(state)
        try:
            ctrl = dev.read_reg(ADDR_CTRL, 4)
            twd = dev.read_reg(ADDR_TWD_MS, 4)
            trst = dev.read_reg(ADDR_TRST_MS, 4)
            arm = dev.read_reg(ADDR_ARM_DELAY_US, 2)
            status_raw = dev.read_reg(ADDR_STATUS, 4)
            status = Status(status_raw)
        finally:
            dev.close()
        
        lines = [
            f"0x00 [CTRL]       : 0x{ctrl:08X}",
            f"0x04 [TWD_MS]     : {twd} ms",
            f"0x08 [TRST_MS]    : {trst} ms",
            f"0x0C [ARM_US]     : {arm} us",
...
def action_boundary_stress_test(state: AppState) -> None:
    ...
    try:
        dev = connect_once(state)
        try:
            dev.write_reg(ADDR_TWD_MS, twd, 4)
            dev.write_reg(ADDR_CTRL, 0x00000003, 4) # Enable + UART source
            time.sleep(0.1)
            
            print(f"\n--- BÀI 1: Kick ở {test_safe}ms (Sớm hơn 50ms) -> Kỳ vọng: KHÔNG FAULT ---")
            dev.kick()
            time.sleep(test_safe / 1000.0)
            status = dev.get_status()
            ...
            print(f"\n--- BÀI 2: Kick ở {test_fail}ms (Trễ 50ms) -> Kỳ vọng: CÓ FAULT ---")
            dev.write_reg(ADDR_CTRL, 0x00000004, 4) # Disable + Clear
            time.sleep(0.1)
            dev.write_reg(ADDR_CTRL, 0x00000003, 4) # Enable lại
            time.sleep(0.1)
            
            dev.kick()
            time.sleep(test_fail / 1000.0)
            status = dev.get_status()
...
def action_fault_injection(state: AppState) -> None:
    ...
    try:
        dev = connect_once(state)
        try:
            print(color("\n1. Bơm rác vào buffer (Gửi byte 0x66 thay vì 0x55)...", "yellow"))
            dev.ser.write(b'\x66\x03\x00\x00\x03') 
            dev.ser.flush()
            time.sleep(0.5) 
            ...
            frame[-1] ^= 0xFF # Lật ngược toàn bộ bit checksum
            dev.ser.write(frame)
            dev.ser.flush()
            try:
            ...
            try:
                dev.transact(CMD_READ_REG, 0x99, b"")
                print(color("=> FAIL: Đọc địa chỉ láo mà vẫn thành công?", "red"))
    ...
    try:
        ...
        while True:
            try:
                ...
                kick_count += 1
                status = Status(int.from_bytes(rsp.payload, "big")) if len(rsp.payload) == 4 else dev.get_status()
                if status.fault_active: fault_count += 1
            ...
            sys.stdout.write(
                f"\r[UPTIME: {color(run_time, 'cyan')}] "
                f"Kicks UART: {kick_count} | "
                f"Trượt (Lỗi Fault): {color(fault_count, 'red' if fault_count else 'green')} | "
                f"Lỗi Truyền thông: {color(err_count, 'red' if err_count else 'green')}   "
            )
            sys.stdout.flush()
...
def action_arm_delay_test(state: AppState) -> None:
    ...
    print("Mô phỏng: Tạm thời nâng arm_delay lên 20ms để UART kịp gửi lệnh kick.")
    try:
        dev = connect_once(state)
        try:
            # Tạm thời nâng arm_delay lên 20000us (20ms) và tWD = 500ms
            test_arm_us = 20000
            test_twd = 500
            dev.write_reg(ADDR_ARM_DELAY_US, test_arm_us, 2)
            dev.write_reg(ADDR_TWD_MS, test_twd, 4)
            dev.write_reg(ADDR_CTRL, 0x00000000, 4) # Disable trước
            time.sleep(0.1)
            
            print(f"1. Enable Watchdog và Kick NGAY LẬP TỨC (trong vòng {test_arm_us}us)...", "yellow")
            dev.write_reg(ADDR_CTRL, 0x00000003, 4) # Enable + UART_SRC
            dev.kick() # Lệnh kick này mất khoảng ~0.4ms để truyền, sẽ lọt vào cửa sổ mù
            
            print(f"2. Chờ {test_twd}ms xem lệnh kick có bị phớt lờ và sinh Fault không...")
            time.sleep((test_twd / 1000.0) + 0.1)
...
def action_uart_fuzzing_test(state: AppState) -> None:
    ...
    try:
        dev = connect_once(state)
        try:
            print(color("1. Gửi Frame sai LENGTH (Khai báo LEN=10 nhưng chỉ gửi 2 byte data)...", "yellow"))
            dev.ser.write(b'\x55\x03\x00\x0A\xAA\xBB') 
            dev.ser.flush()
            time.sleep(0.2)
            
            print(color("2. Bơm liên tục byte SYNC (0x55) rác để làm rối FSM...", "yellow"))
            dev.ser.write(b'\x55\x55\x55\x55\x55\x55\x55\x55') 
            dev.ser.flush()
            time.sleep(0.2)
            
            print(color("3. Kiểm tra xem FPGA có bị treo không...", "yellow"))
            try:
                status = dev.get_status()
...
def action_time_sweep_test(state: AppState) -> None:
    ...
    try:
        dev = connect_once(state)
        try:
            for twd in range(200, 1001, 200):
                print(f"\n--- Thử nghiệm với tWD = {twd} ms ---")
                dev.write_reg(ADDR_TWD_MS, twd, 4)
                dev.write_reg(ADDR_CTRL, 0x00000004, 4) # Clear
                dev.write_reg(ADDR_CTRL, 0x00000003, 4) # Enable
                
                wait_time = (twd + 20) / 1000.0 # Chờ vừa lố deadline 20ms
...
def action_debug_menu(state: AppState) -> None:
    while True:
        ...
        lines = [
            ...
            "",
            color("TEST ĐỘ TIN CẬY (RELIABILITY & STRESS)", "bold"),
            f"{hotkey('3')} Register Dump (Chụp Memory Map)",
            f"{hotkey('4')} Boundary Stress Test (Ép biên thời gian)",
            f"{hotkey('5')} UART Fault Injection (Bơm nhiễu/Rác đường truyền)",
            f"{hotkey('6')} Overnight Soak Test (Kiểm thử ngâm độ bền)",
            f"{hotkey('7')} Arm Delay Test (Test cửa sổ mù ignore WDI)",
            f"{hotkey('8')} UART Fuzzing (Test chống tràn/treo FSM)",
            f"{hotkey('9')} Write-1-to-Clear Test (Test reset thanh ghi)",
            f"{hotkey('10')} Time Sweep Test (Quét dải thời gian tuyến tính)",
            "",
            f"{hotkey('0')} Quay lại menu chính"
        ]
        print_box("MENU DEBUG / NÂNG CAO CHUYÊN SÂU", lines)
        ...
        if c == '1':
            ...
            action_register_dump(state)
            pause()
        elif c == '4':
            action_boundary_stress_test(state)
            pause()
        elif c == '5':
            action_fault_injection(state)
            pause()
        elif c == '6':
            action_soak_test(state)
            pause()
        elif c == '7':
            action_arm_delay_test(state)
            pause()
        elif c == '8':
            action_uart_fuzzing_test(state)
            pause()
        elif c == '9':
            action_write_1_to_clear_test(state)
            pause()
        elif c == '10':
            action_time_sweep_test(state)
            pause()
...
def test_connection(state: AppState) -> None:
    try:
        dev = connect_once(state)
        try:
            # Ép ghi thông số mặc định theo báo cáo xuống board ngay khi mở
            dev.write_reg(ADDR_TWD_MS, state.twd_ms, 4)
            dev.write_reg(ADDR_TRST_MS, state.trst_ms, 4)
            dev.write_reg(ADDR_ARM_DELAY_US, state.arm_us, 2)
            status = dev.get_status()
            ...
            dev.close()
        state.mark("uart_connect", "PASS", f"Nạp cấu hình mặc định (tWD={state.twd_ms}, tRST={state.trst_ms})")
    except Exception as e:
        msg = handle_serial_error(e)
...
def main() -> int:
    ...
    while not port:
        ...
        if not ports:
            ...
            print("  [R] Quét lại (Refresh)")
            print("  [0] Nhập tay thủ công")
            c = input("Chọn [R/0]: ").strip().upper()
            if c == "R":
                continue
            elif c == "0":
                port = input("Nhập cổng COM (VD: COM9): ").strip()
        else:
            ...
            raw = input("Chọn cổng COM: ").strip().upper()
            if raw == "R":
                continue
            elif raw == "0":
    ...
    # Tắt load_config để luôn nạp mặc định 1600/200/150
    # load_config(state) 
